...
export default function LastTwelveMonths() {
  const [lastTwelveMonths, setLastTwelveMonths] = useState(null);

  useEffect(() => {   
    async function getLastTwelveMonths() {
      const resp = await fetch("http://localhost:3000/api/v1/last-12-months")
      const objects = await (resp.json())
      setLastTwelveMonths(objects.data);
    }
    getLastTwelveMonths();
  }, [])

  if (lastTwelveMonths === null) {
